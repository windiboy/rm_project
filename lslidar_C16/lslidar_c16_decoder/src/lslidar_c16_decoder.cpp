...
//    VPointCloud::Ptr point_cloud(new VPointCloud());
    ...
    for (size_t i = 0; i < 16; ++i) {
        ...
        for (j = 1; j < scan.points.size()-1; ++j) {
            ...
            // point.x = scan.points[j].x*0.25;
            // point.y = scan.points[j].y*0.25;
            // point.z = scan.points[j].z*0.25;
            // point.intensity = scan.points[j].intensity*0.25;
...
//        printf("\n");
        ...
        // scan->ranges[point_num - 1-point_idx] = sweep_data->scans[layer_num_local].points[i].distance*0.25;//单位m
        // scan->intensities[point_num - 1-point_idx] = sweep_data->scans[layer_num_local].points[i].intensity*0.25;
    }
