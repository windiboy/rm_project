...
int main(int argc, char** argv)
{
    ...
    //timer,5ms
    State_Timer = nh.createTimer(ros::Duration(0.005), timer_callback);
    joint_pub = nh.advertise<sensor_msgs::JointState>("/joint_states", 1);
    target_pub = nh.advertise<rm_msgs::JointPos>("/rm_driver/JointPos", 1);
    // 定义一个服务器
    Server server(nh, "rm_arm/follow_joint_trajectory", boost::bind(&execute, _1, &server), false);
    // 服务器开始运行
    server.start();
    ros::spin();
